...
# Reactor TODO App Demo Runner with ClojureScript REPL
# Starts server, client build, and REPL

echo "🚀 Starting Reactor TODO App with ClojureScript REPL"
echo "====================================================="
echo ""
...
# Start ClojureScript build with watch and server
echo -e "${GREEN}Starting ClojureScript build with watch mode...${NC}"
lein run -m examples.todo-app.build auto 2>&1 | sed "s/^/[CLJS] /" &
CLJS_PID=$!
echo "ClojureScript build PID: $CLJS_PID"
...
echo -e "${GREEN}Starting ClojureScript REPL...${NC}"
echo -e "${YELLOW}The REPL will be available after the page loads.${NC}"
echo -e "${YELLOW}Make sure to open http://localhost:8083/todo.html in your browser!${NC}"
echo ""
...
# Start the REPL (this will block)
echo -e "${GREEN}Launching ClojureScript REPL...${NC}"
echo -e "${YELLOW}Once connected, you can evaluate ClojureScript in the browser context.${NC}"
lein run -m examples.todo-app.repl