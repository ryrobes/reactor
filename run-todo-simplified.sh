...
# Reactor TODO App Demo Runner (Simplified - lein-cljsbuild only)
# Starts both server and client without shadow-cljs

echo "🚀 Starting Reactor TODO App (Simplified ClojureScript Build)"
echo "============================================================"
echo ""
...
# Build ClojureScript once first
echo -e "${GREEN}Building ClojureScript...${NC}"
echo -e "${YELLOW}Note: First build may take a minute to download dependencies${NC}"
lein cljsbuild once todo 2>&1 | sed "s/^/[BUILD] /"
...
# Start ClojureScript auto-build with file watching
echo -e "${GREEN}Starting ClojureScript auto-build...${NC}"
echo -e "${BLUE}Watching for file changes...${NC}"

# Using lein-cljsbuild auto mode for file watching and rebuilding
lein cljsbuild auto todo 2>&1 | sed "s/^/[CLJS] /" &
CLJS_PID=$!
echo "ClojureScript watcher PID: $CLJS_PID"
